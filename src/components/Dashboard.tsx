      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          ...
          <button
            ...
            className="btn-secondary flex items-center space-x-2"
          >
            <span>נקה localStorage</span>
          </button>
          <button
            onClick={() => {
              if (confirm('האם אתה בטוח שברצונך לעדכן את כל התבניות באירועים הקיימים להשתמש בטמפלט החדש?')) {
                try {
                  updateExistingEventsCampaigns();
            ...
            onClick={() => {
              ...
              if (stored) {
                ...
                if (parsed.state?.events) {
                  ...
                  alert(`נמצאו ${parsed.state.events.length} אירועים ב-localStorage. ראה קונסול לפרטים.`);
                } else {
                  alert('לא נמצאו אירועים ב-localStorage');
                }
              } else {
                alert('לא נמצאו נתונים ב-localStorage');
              }
            }}
            className="btn-secondary flex items-center space-x-2"
          >
            <span>בדוק localStorage</span>
          </button>
          <Link
            to="/calendar"
            className="btn-secondary flex items-center space-x-2"
