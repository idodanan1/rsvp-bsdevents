...
const syncGuestsDirectly = async (eventId: string, guests: Guest[]): Promise<boolean> => {
  ...
  try {
    console.log(`?ƒó? Syncing ${guests.length} guests directly to API for event ${eventId}...`);
    ...
    if (response.ok) {
      console.log(`?£å Successfully synced ${guests.length} guests directly to API`);
      return true;
    } else if (response.status === 413) {
      // Payload too large - split into chunks of 100 guests each
      console.log(`?ú???ï Payload too large (413), splitting into chunks...`);
      const CHUNK_SIZE = 100;
      let allSynced = true;
      
      for (let i = 0; i < guests.length; i += CHUNK_SIZE) {
        const chunk = guests.slice(i, i + CHUNK_SIZE);
        console.log(`?ƒó? Syncing chunk ${Math.floor(i / CHUNK_SIZE) + 1}/${Math.ceil(guests.length / CHUNK_SIZE)} (${chunk.length} guests)...`);
        ...
        if (!chunkResponse.ok) {
          console.warn(`?ú???ï Failed to sync chunk ${Math.floor(i / CHUNK_SIZE) + 1}:`, chunkResponse.status);
          allSynced = false;
        } else {
          console.log(`?£å Synced chunk ${Math.floor(i / CHUNK_SIZE) + 1} successfully`);
        }
      ...
      if (allSynced) {
        console.log(`?£å Successfully synced all ${guests.length} guests in chunks`);
        return true;
      } else {
        console.warn(`?ú???ï Some chunks failed to sync`);
        return false;
      }
    } else {
      const errorText = await response.text();
      console.warn(`?ú???ï Failed to sync guests directly:`, response.status, errorText);
      return false;
    }
  } catch (error) {
    console.warn('?ú???ï Failed to sync guests directly:', error);
    return false;
  }
};
...
const syncEventToAPI = async (event: Event, retries = 3): Promise<void> => {
  ...
  try {
    ...
    console.log(`?ƒó? Syncing FULL event to API:`, {
      eventId: event.id,
    ...
    if (response.ok) {
      console.log('?£å FULL event synced to API successfully:', { 
        eventId: event.id,
        guestsCount: event.guests?.length || 0
      });
    } else {
      const errorText = await response.text();
      console.warn('?ú???ï API sync failed:', response.status, errorText);
      
      // CRITICAL: If sync failed but we have guests, try syncing guests directly as fallback
      if (event.guests && event.guests.length > 0) {
        console.log(`?ƒôä Trying to sync guests directly as fallback...`);
        const guestsSynced = await syncGuestsDirectly(event.id, event.guests);
        if (guestsSynced) {
          console.log('?£å Guests synced directly, but event details may not be updated');
      ...
      if (response.status === 413 && retries > 0) {
        console.log(`?ƒôä 413 error - trying event details only (${retries} retries left)...`);
        ...
        if (retryResponse.ok) {
          console.log('?£å Event synced with details only');
      ...
      if (retries > 0) {
        console.log(`?ƒôä Retrying sync (${retries} retries left)...`);
        await new Promise(resolve => setTimeout(resolve, 1000));
        return syncEventToAPI(event, retries - 1);
      }
    }
  } catch (error) {
    console.warn('?ú???ï Failed to sync event to API:', error);
...
class GuestUpdateBatchProcessor {
  ...
  addUpdate(update: PendingGuestUpdate): void {
    ...
    if (cachedTime && (now - cachedTime) < this.CACHE_TTL_MS) {
      console.log('?ƒú½ Skipping duplicate update (cached):', cacheKey);
      return;
    }
    ...
    console.log(`?ƒó? Added update to batch queue (${this.updateQueue.length} pending)`);
  ...
  private async flushBatch(): Promise<void> {
    ...
    console.log(`?ƒó? Sending batch of ${updatesToSend.length} guest updates...`);
      ...
      await Promise.all(
        batch.map(async (update) => {
          try {
            ...
            if (response.ok) {
              console.log(`?£å Batch update sent successfully for guest ${update.guestId}`);
            } else {
              const errorText = await response.text();
              console.warn(`?ú???ï Batch update failed for guest ${update.guestId}:`, response.status, errorText);
            }
          } catch (error) {
            console.warn(`?ú???ï Batch update error for guest ${update.guestId}:`, error);
          }
        })
      );
    ...
    console.log(`?£å Batch of ${updatesToSend.length} updates completed`);
  }
  ...
  persist(
    (set, get) => ({
      ...
      fetchEvents: async (forceRefresh: boolean = false, silent: boolean = false) => {
        ...
        if (userStorage) {
          try {
            ...
            console.warn('?ú???ï Error parsing user storage:', e);
          }
        }
        ...
        try {
          ...
          if (userId) {
            ...
            if (forceRefresh || !useCache || true) { // Always fetch from API
              try {
                console.log(`?ƒôí Fetching events from API for userId: ${userId}, URL: ${BACKEND_URL}/api/events/${userId}`);
                const response = await fetch(`${BACKEND_URL}/api/events/${userId}`);
                if (response.ok) {
                  const data = await response.json();
                  console.log(`?ƒó? API response received:`, {
                  ...
                  if (apiEvents.length === 0) {
                    console.warn(`?ú???ï API returned empty events array for userId: ${userId}. This could mean:`);
                ...
                if (stored) {
                  try {
                        ...
                        guests: event.guests
                          ?.filter((guest: Guest) => {
                            ...
                            if (deletedGuestIds.includes(guest.id)) {
                              console.log(`?ƒú½ Filtering out deleted guest from localStorage load: ${guest.firstName} ${guest.lastName} (${guest.id})`);
                              return false;
                            }
                            return true;
                          })
                    ...
                    console.warn('?ú???ï Error parsing local events:', e);
                  }
                }
                ...
                apiEvents = apiEvents.filter((apiEvent: Event) => {
                  if (deletedEventIds.has(apiEvent.id)) {
                    console.log(`?ƒú½ Filtering out deleted event ${apiEvent.id} (${apiEvent.coupleName}) from API response`);
                    return false;
                  }
                  return true;
                });
                if (originalApiEventsCount > apiEvents.length) {
                  console.log(`?ƒú½ Filtered out ${originalApiEventsCount - apiEvents.length} deleted event(s) from API response`);
                }
                ...
                const allEvents = apiEvents.map(apiEvent => {
                  ...
                  if (cleanedInvitationImageUrl && cleanedInvitationImageUrl.startsWith('file://')) {
                    console.warn('?ú???ï Removing local file path from invitationImageUrl:', cleanedInvitationImageUrl);
                  ...
                  return {
                    ...
                    eventTypeHebrew: apiEvent.eventTypeHebrew || '?÷?¬?õ???ô',
                    guests: (apiEvent.guests || [])
                      .filter((g: Guest) => {
                        ...
                        if (deletedGuestIds.includes(g.id)) {
                          console.log(`?ƒú½ Skipping deleted guest from API: ${g.firstName} ${g.lastName} (${g.id})`);
                          return false;
                        }
                        return true;
                      })
                      .map((g: Guest) => {
                        ...
                        // CRITICAL: Check if this is a problematic guest (?ó?õ?¿?õ?ƒ ???õ?????ù, ???ð?õ?¿ ?¿?õ?????õ, ???ù?ó?õ ?ó???ƒ)
                        const isProblematicGuest = (g.firstName?.includes('?ó?õ?¿?õ?ƒ') && g.lastName?.includes('???õ?????ù')) ||
                                                  (g.firstName?.includes('???ð?õ?¿') && g.lastName?.includes('?¿?õ?????õ')) ||
                                                  (g.firstName?.includes('???ù?ó?õ') && g.lastName?.includes('?ó???ƒ'));
                        
                        if (isProblematicGuest) {
                          console.log(`?ƒôí PROBLEMATIC GUEST IN FETCH: ${g.firstName} ${g.lastName}`, {
                            id: g.id,
                        ...
                        if (existingGuest) {
                          ...
                          if (isApiFromGuestLink) {
                            const isProblematicGuest = (g.firstName?.includes('?ó?õ?¿?õ?ƒ') && g.lastName?.includes('???õ?????ù')) ||
                                                      (g.firstName?.includes('???ð?õ?¿') && g.lastName?.includes('?¿?õ?????õ')) ||
                                                      (g.firstName?.includes('???ù?ó?õ') && g.lastName?.includes('?ó???ƒ'));
                            console.log(`?ƒôí DETECTED guest_link update in fetchEvents for guest ${g.id} (${g.firstName} ${g.lastName}):`, {
                              apiRsvpStatus: g.rsvpStatus,
                              apiGuestCount: g.guestCount,
                              apiSource: g.source,
                            ...
                            if (isProblematicGuest) {
                              console.log(`?ƒôí PROBLEMATIC GUEST DETECTED IN FETCH: ${g.firstName} ${g.lastName}`, {
                                id: g.id,
                                apiRsvpStatus: g.rsvpStatus,
                                apiGuestCount: g.guestCount,
                                existingRsvpStatus: existingGuest.rsvpStatus,
                          ...
                          if (isApiFromGuestLink) {
                            ...
                            const isProblematicGuest = (g.firstName?.includes('?ó?õ?¿?õ?ƒ') && g.lastName?.includes('???õ?????ù')) ||
                                                      (g.firstName?.includes('???ð?õ?¿') && g.lastName?.includes('?¿?õ?????õ')) ||
                                                      (g.firstName?.includes('???ù?ó?õ') && g.lastName?.includes('?ó???ƒ'));
                            console.log(`?£å Using guest_link update from API for guest ${g.id} (${g.firstName} ${g.lastName}):`, {
                            ...
                            if (statusChanged) {
                              console.log(`?ƒôä STATUS CHANGE: ${existingGuest.rsvpStatus} -> ${g.rsvpStatus} for guest ${g.id} (${g.firstName} ${g.lastName})`);
                            }
                            if (countChanged) {
                              console.log(`?ƒôä COUNT CHANGE: ${existingGuest.guestCount} -> ${g.guestCount} for guest ${g.id} (${g.firstName} ${g.lastName})`);
                            }
                            if (isProblematicGuest) {
                              console.log(`?ƒôí PROBLEMATIC GUEST UPDATE APPLIED: ${g.firstName} ${g.lastName} - status: ${g.rsvpStatus}, count: ${g.guestCount}, source: ${g.source}`);
                              console.log(`?ƒôí BEFORE: status=${existingGuest.rsvpStatus}, count=${existingGuest.guestCount}`);
                              console.log(`?ƒôí AFTER: status=${g.rsvpStatus}, count=${g.guestCount}`);
                            }
                            const updatedGuest = {
                              ...g,
                            ...
                            if (isProblematicGuest) {
                              console.log(`?ƒôí RETURNING UPDATED GUEST:`, {
                                id: updatedGuest.id,
                          ...
                          if (isApiFromManual && g.guestCount !== undefined) {
                            ...
                            if (apiDate >= existingDate || !isExistingFromManual) {
                              console.log(`?£å Using manual_update guestCount from API: ${g.guestCount} (overriding local: ${existingGuest.guestCount}, API is ${apiDate >= existingDate ? 'newer or equal' : 'from server'})`);
                              return {
                                ...g,
                                firstName: cleanName(g.firstName),
                                lastName: cleanName(g.lastName),
                          ...
                          if (isExistingFromManual && existingGuest.guestCount !== undefined && !isApiFromManual) {
                            console.log(`?ƒ¢???ï Preserving manual guestCount ${existingGuest.guestCount} from local state (API has ${g.guestCount} but not from manual_update)`);
                            return {
                              ...g,
                              firstName: cleanName(g.firstName),
                              lastName: cleanName(g.lastName),
                ...
                if (localOnlyEvents.length > 0) {
                  console.log(`?ƒôä Found ${localOnlyEvents.length} local events not in API - syncing to server...`);
                  const syncPromises = localOnlyEvents.map((event, index) => 
                    syncEventToAPI(event)
                      .then(() => {
                        console.log(`?£å Event ${index + 1}/${localOnlyEvents.length} synced successfully: ${event.id}`);
                        return true;
                      })
                      .catch(err => {
                        console.warn(`?ú???ï Failed to sync local event ${event.id} to server:`, err);
                        return false;
                      })
                  );
                  ...
                  Promise.all(syncPromises).then(results => {
                    ...
                    if (successCount > 0) {
                      console.log(`?£å Successfully synced ${successCount}/${localOnlyEvents.length} event(s) to server. Refreshing to get updated data...`);
                      // Refresh after a short delay to allow server to process
                      setTimeout(() => {
                        console.log(`?ƒôä Refreshing events from API after sync...`);
                        fetchEventsRef(true, true).catch(err => {
                          console.warn('?ú???ï Failed to refresh after sync:', err);
                        });
                ...
                if (apiEvents.length === 0 && localEvents.length > 0) {
                  console.warn('?ú???ï API returned empty events but local events exist - using local events as fallback');
                  console.log(`?ƒóê Local events count: ${localEvents.length}, UserId: ${userId}`);
                  console.log(`?ƒò? This is normal if:`);
                  ...
                  console.log(`?ƒôä Attempting to sync local events to server...`);
                  ...
                  const localEventsForUser = localEvents.filter((e: Event) => {
                    ...
                    if (deletedEventIdsForPreserve.has(e.id)) {
                      console.log(`?ƒú½ Filtering out deleted event ${e.id} (${e.coupleName}) from preserved local events`);
                      return false;
                    }
                    return !userId || e.userId === userId;
                  });
                  if (localEventsForUser.length > 0) {
                    console.log(`?ƒ¢???ï Preserving ${localEventsForUser.length} local events (API returned empty)`);
                ...
                if (lostEvents.length > 0) {
                  console.warn(`?ú???ï CRITICAL: About to lose ${lostEvents.length} events for current user! Preserving them...`);
                  console.warn('?ú???ï Lost events:', lostEvents.map(e => ({ id: e.id, userId: e.userId, name: e.coupleName })));
                  ...
                  lostEvents.forEach(lostEvent => {
                    if (!eventsToSave.find(e => e.id === lostEvent.id)) {
                      ...
                      if (!userId || lostEvent.userId === userId || !lostEvent.userId || lostEvent.userId === 'anonymous') {
                        ...
                        console.log(`?£å Preserved lost event: ${lostEvent.id} (belongs to current user)`);
                      } else {
                        console.log(`?ï¡??ï Skipping event ${lostEvent.id} - belongs to different user (${lostEvent.userId} vs ${userId})`);
                      }
                    }
                  });
                } else {
                  ...
                  if (otherUserEvents.length > 0) {
                    console.log(`?ä???ï Found ${otherUserEvents.length} events from other users (not preserving):`, 
                ...
                const eventsToSaveFiltered = userId 
                  ? eventsToSave.filter((e: Event) => {
                      ...
                      if (deletedEventIdsForSave.has(e.id)) {
                        console.log(`?ƒú½ Filtering out deleted event ${e.id} (${e.coupleName}) before save`);
                        return false;
                      }
                ...
                if (filteredEvents.length === 0 && localEvents.length > 0) {
                  console.warn('?ú???ï Filtered events is empty but local events exist - preserving local events');
                  console.log('?ƒôí Debug info:', {
                    userId: userId,
                  ...
                  const eventsToShow = localEvents
                    ...
                    .map((e: Event) => {
                      ...
                      if ((!e.userId || e.userId === 'anonymous') && userId) {
                        console.log(`?ƒôä Updating event ${e.id} userId from "${e.userId || 'missing'}" to "${userId}" (was anonymous/missing)`);
                  ...
                  const localEventsForUser = eventsToShow.filter((e: Event) => {
                    ...
                    if (deletedEventIds.has(e.id)) {
                      console.log(`?ƒú½ Filtering out deleted event ${e.id} (${e.coupleName}) from localEventsForUser`);
                      return false;
                    }
                    // Exclude admin events for regular users
                    if (e.userId === 'admin-fixed-id' && userId !== 'admin-fixed-id') {
                  ...
                  if (localEventsForUser.length > 0) {
                    console.log(`?ƒ¢???ï Preserving ${localEventsForUser.length} local events (filtered was empty)`);
                    ...
                    console.error('?¥ì CRITICAL: No events match userId even after update!', {
                      userId,
                ...
                const uniqueEvents = filteredEvents.reduce((acc, event) => {
                  ...
                  if (existingIndex >= 0) {
                    ...
                    if (newTime > existingTime) {
                      console.warn(`?ú???ï Found duplicate event ${event.id}, keeping more recent version`);
                      acc[existingIndex] = event; // Replace with more recent version
                    } else {
                      console.warn(`?ú???ï Found duplicate event ${event.id}, keeping existing version`);
                    }
                  } else {
                    acc.push(event);
                  }
                ...
                if (uniqueEvents.length < filteredEvents.length) {
                  console.warn(`?ú???ï Removed ${filteredEvents.length - uniqueEvents.length} duplicate event(s)`);
                }
                ...
                if (storeState.currentEvent) {
                  ...
                  if (updatedEvent) {
                    ...
                    console.log('?ƒôä Updated currentEvent from API fetch:', updatedCurrentEvent.id, 'guests:', updatedCurrentEvent.guests?.length);
                    ...
                    console.log('?ƒôí Sample guests in updated currentEvent:', sampleGuests);
                    // Check for specific problematic guests
                    const problematicGuests = updatedCurrentEvent.guests?.filter(g => 
                      (g.firstName?.includes('?ó?õ?¿?õ?ƒ') && g.lastName?.includes('???õ?????ù')) ||
                      (g.firstName?.includes('???ð?õ?¿') && g.lastName?.includes('?¿?õ?????õ'))
                    );
                    if (problematicGuests && problematicGuests.length > 0) {
                      problematicGuests.forEach(guest => {
                        console.log(`?ƒôí Found problematic guest in updated currentEvent: ${guest.firstName} ${guest.lastName}`, {
                          id: guest.id,
                      ...
                      console.log('?ú???ï No problematic guests found in updated currentEvent');
                    }
                  } else {
                    console.warn('?ú???ï currentEvent not found in eventsWithNewReferences:', storeState.currentEvent.id);
                  }
                }
                ...
                console.warn('?ú???ï API fetch failed, using localStorage');
                apiError = true;
              }
            } catch (error) {
              console.warn('?ú???ï API not available, using localStorage:', error);
              apiError = true;
            }
          }
          }
            ...
            if (parsed.state && parsed.state.events && parsed.state.events.length > 0) {
              ...
              if (userId) {
                ...
                const updatedEvents = allEvents.map((event: Event) => {
                  ...
                  if (event.userEmail && userEmail && event.userEmail.toLowerCase().trim() === userEmail.toLowerCase().trim() && event.userId !== userId) {
                    console.log(`?ƒôä Updating event ${event.id} userId from "${event.userId}" to "${userId}" (email match)`);
                    eventsUpdated = true;
                    return { ...event, userId };
                  }
                  ...
                  if ((!event.userId || event.userId === 'anonymous') && userId) {
                    console.log(`?ƒôä Updating event ${event.id} userId from "${event.userId || 'missing'}" to "${userId}" (was anonymous/missing)`);
                    eventsUpdated = true;
                    ...
                    if (oldUserStorage) {
                      try {
                        ...
                        if (oldUser && oldUser.email && userEmail && oldUser.email.toLowerCase().trim() === userEmail.toLowerCase().trim()) {
                          console.log(`?ƒôä Updating event ${event.id} userId from "${event.userId}" to "${userId}" (found matching email in old users)`);
                          eventsUpdated = true;
                ...
                if (eventsUpdated) {
                  console.log('?£å Updated events with correct userId');
                  // Save updated events
                  localStorage.setItem('rsvp-events-storage', JSON.stringify({
                    state: {
                  ...
                  if (mismatchedEvents.length > 0) {
                    console.warn('?ú???ï Found events with different userId:', mismatchedEvents.map(e => ({ id: e.id, userId: e.userId, name: e.coupleName })));
                  }
                }
              }
              ...
              if (userId) {
                ...
                const updatedEvents = allEvents.map((event: Event) => {
                  if (!event.userId || event.userId === 'anonymous' || event.userId !== userId) {
                    ...
                    if ((!event.userId || event.userId === 'anonymous') && userId) {
                      console.log(`?ƒôä Updating event ${event.id} userId from "${event.userId}" to "${userId}" (was anonymous/missing)`);
                      eventsUpdated = true;
                      return { ...event, userId, userEmail: userEmail || event.userEmail };
                    }
                  }
              ...
              if (userId) {
                ...
                if (isAdmin) {
                  ...
                  console.log('?ƒññ Admin user - showing all events');
                } else {
                  ...
                  console.log('?ƒôí Filtering events by userId:', {
                    userId,
              ...
              console.log('?ƒóë Total events in storage:', allEvents.length);
              console.log('?ƒóë Filtered events for user:', filteredEvents.length, 'userId:', userId);
              ...
              // CRITICAL: Create new array reference to force React re-render
              console.log('?ƒôä Creating new events array reference from localStorage to force React re-render');
              ...
              if (storeStateForLocalStorage.currentEvent) {
                ...
                if (updatedEvent) {
                  ...
                  console.log('?ƒôä Updated currentEvent from localStorage fetch:', updatedCurrentEvent.id, 'guests:', updatedCurrentEvent.guests?.length);
                }
              }
          ...
          console.log('?ƒó¥ No events found in localStorage');
          // CRITICAL: Create new array reference (even if empty) to force React re-render
          set({ events: [], isLoading: false });
        } catch (error) {
          console.error('?¥ì Error fetching events:', error);
          set({ error: '???ò?ù?ð?ô ?ñ?ø???ù???¬ ?ô?ð?ù?¿?õ???ù?¥', isLoading: false });
        } finally {
      ...
      createEvent: async (eventData) => {
        console.log('?ƒôí createEvent called with:', eventData);
        set({ isLoading: true, error: null });
        try {
          // CRITICAL: Clean invitationImageUrl - remove local file paths
          ...
          const defaultCampaigns: Campaign[] = [
            {
              ...
              eventId: eventId,
              name: '?ô?ö?????ô ?¿?ð???õ???ù?¬',
              message: `?ƒîé ???£?õ?¥ {{guest_name}}!

?ð???÷???õ ?????÷?ù?¥ ?£?ô?ö???ù?ƒ ?ð?õ?¬?ú ?£?÷?¬?õ???ô ???£ {{couple_name}}!

?ƒóå ?¬?ð?¿?ù?ú: {{event_date}}
?ƒõð ?????ô: {{event_time}}
?ƒóí ???ù???õ?¥: {{venue}}
...
{{couple_name}} ?ƒòõ`
            },
            {
              id: generateId(),
              name: '?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô',
              message: `?ƒîé ???£?õ?¥ {{first_name}}! 

???÷?¿ ?ö?ô ???õ?¿?ô! ?ô{{event_type}} ???£ {{couple_name}}! 

?ƒóå ?¬?ð?¿?ù?ú: {{event_date}}
?ƒõð ?????ô: {{event_time}}
?ƒóí ???ù???õ?¥: {{venue}}
?ƒ¬ñ ???õ?£?÷?ƒ: {{table_number}}

?ð???ð ?ô?ò?ù???õ 15 ?ó???õ?¬ ?£?????ù ?ô?ö???ƒ.

?ƒô÷ ?£???ó?¢?ƒ ???ø?ø?õ?? ?ô?ô?ò???ô ?£?÷??

?£?ð ?£???¢?õ?÷ ?£?ô?ñ?ù?ð ?????ñ ?¿?õ?÷ ?ø?õ?ñ! ?ƒøê`,
              channel: 'whatsapp' as const,
...
{{couple_name}}`
            ...
            {
              id: generateId(),
              name: '?¬?ö?¢?õ?¿?¬ ?ù?õ?¥ ?ô?ð?ù?¿?õ??',
              message: `?ƒîé ???£?õ?¥ {{guest_name}}!
???÷?¿ ?ö?ô ???õ?¿?ô! ?ô?÷?¬?õ???ô ???£ {{couple_name}}!
...
?£?ð ?£???¢?õ?÷ ?£?ô?ñ?ù?ð ?????ñ ?¿?õ?÷ ?ø?õ?ñ! ?ƒøê`
            ...
            {
              id: generateId(),
              name: '?ô?õ?ó???¬ ?¬?õ?ó?ô ?£???ò?ù???ù?¥',
              message: `?ƒùï ???£?õ?¥ {{guest_name}}! 

?¬?õ?ó?ô ?¿?ñ?ô ???ô?ò???¬ ?£{{event_type}} ???£ {{couple_name}}! 

?ô?ù?ô ?£???õ ?¢?ù?? ?£?¿?ð?õ?¬ ?ð?õ?¬?ú ?õ?£?ô?ù?õ?¬ ?ð?ù?¬???õ ?ñ?ù?õ?¥ ?ô???ù?õ?÷?ó ?ô?ö?ô.

?¬?õ?ó?ô ???£ ?ô?ñ?¿?¢?õ?¬ ?õ?ô???¬???õ?¬! ?ƒò¥

?¬???õ???õ?¬ ???ô?ð?ù?¿?õ?? ?ù?õ???£?õ ?ñ???¿?õ?ñ.
...
{{couple_name}}`
          ...
          if (userStorage) {
            ...
            userId = parsed.state?.user?.id || '';
            userEmail = parsed.state?.user?.email || '';
              console.log('?ƒñ? Current user info:', { userId, userEmail });
            } catch (e) {
              console.error('?¥ì Error parsing user storage:', e);
            }
          }
          
          // CRITICAL: If no userId found, log warning
          if (!userId) {
            console.warn('?ú???ï WARNING: No userId found! Event will be created with "anonymous" userId');
            console.warn('?ú???ï This may cause events to disappear after refresh. Please ensure user is logged in.');
          }
          ...
          console.log('?ƒó¥ New event created with userId:', newEvent.userId, 'userEmail:', newEvent.userEmail);
          ...
          if (duplicateIdEvent) {
            console.error(`?¥ì CRITICAL: Duplicate event ID detected! This should never happen.`);
            console.error(`?¥ì Existing event ID: ${duplicateIdEvent.id}, New event ID: ${newEvent.id}`);
            // Generate a new ID and retry (safety mechanism)
            finalEvent = { ...newEvent, id: generateId() };
            console.log(`?ƒôä Generated new ID for event: ${finalEvent.id}`);
          }
          ...
          if (duplicateContentEvent) {
            console.warn(`?ú???ï Duplicate event content detected! Event ID: ${duplicateContentEvent.id}, New ID: ${finalEvent.id}`);
            console.warn(`?ú???ï Duplicate event details:`, {
            ...
            set({ isLoading: false, error: '?ð?ù?¿?õ?? ?ö?ô?ô ?¢?ñ?¿ ???ù?ù?¥' });
            return;
          }
          
          // CRITICAL: Save to state first
          set(state => {
            console.log('?ƒôí Before createEvent - events count:', state.events.length);
            console.log('?ƒôí Creating event with unique ID:', finalEvent.id);
            ...
            if (existingEvent) {
              console.error(`?¥ì CRITICAL: Event with ID ${finalEvent.id} already exists in state! This should never happen.`);
              // Generate a new ID as last resort
              const retryEvent = { ...finalEvent, id: generateId() };
              console.log(`?ƒôä Generated final new ID for event: ${retryEvent.id}`);
            ...
            console.log('?ƒôí After createEvent - events count:', updatedEvents.length);
            console.log('?ƒôí New event created with 5 default campaigns:', finalEvent);
            return {
              events: updatedEvents,
              isLoading: false
            };
          ...
          try {
            ...
            if (stored) {
              try {
                ...
                console.warn('?ú???ï Error parsing stored events:', e);
              }
            }
            ...
            if (!allEvents.find(e => e.id === finalEvent.id)) {
              ...
              console.log('?ƒò? Saved new event directly to localStorage:', finalEvent.id);
              ...
              console.log('?£å Event saved to localStorage successfully. Total events:', allEvents.length);
              ...
              setTimeout(() => {
                ...
                if (verifyStored) {
                  try {
                    ...
                    if (!eventFound) {
                      console.error('?¥ì CRITICAL: Event was not found in localStorage after save! Re-saving...');
                      // Re-save the event
                      ...
                      console.log('?£å Event re-saved to localStorage');
                    } else {
                      console.log('?£å Verified: Event found in localStorage');
                    }
                  } catch (e) {
                    console.error('?¥ì Error verifying event save:', e);
                  }
                }
              }, 100); // Check after 100ms
            } else {
              console.log('?ú???ï Event already exists in localStorage:', newEvent.id);
            }
          } catch (error) {
            console.error('?¥ì Error saving event to localStorage:', error);
            ...
            try {
              ...
              if (!fallbackEvents.find(e => e.id === newEvent.id)) {
                ...
                console.log('?£å Event saved via fallback method');
              }
            } catch (fallbackError) {
              console.error('?¥ì Fallback save also failed:', fallbackError);
            }
          }
          ...
          try {
            ...
            console.log('?£å Event synced to API successfully with all guests');
          } catch (error) {
            console.error('?¥ì Failed to sync event to API:', error);
            // Continue - localStorage is already updated
            // But log error so user knows sync failed
          }
        } catch (error) {
          set({ error: '???ò?ù?ð?ô ?ñ?ù???ù?¿?¬ ?ô?ð?ù?¿?õ??', isLoading: false });
        }
      },
        ...
        try {
          ...
          if (updates.invitationImageUrl) {
            if (updates.invitationImageUrl.startsWith('file://')) {
              console.warn('?ú???ï Removing local file path from invitationImageUrl:', updates.invitationImageUrl);
          ...
          if (updatedEvent) {
            try {
              ...
              console.log('?£å Event update synced to API successfully with all guests');
            } catch (error) {
              console.warn('?ú???ï Failed to sync event update to API (will use localStorage):', error);
              // Continue - localStorage is already updated by Zustand persist
            }
          }
        } catch (error) {
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ?ô?ð?ù?¿?õ??', isLoading: false });
        }
      },
      ...
      deleteEvent: async (id) => {
        ...
        try {
          ...
          if (eventsToDelete.length > 0) {
            console.log(`?ƒ÷ñ??ï Deleting ${eventsToDelete.length} event(s) with ID ${id}`);
            ...
            try {
              ...
              for (const eventToDelete of eventsToDelete) {
                ...
                if (deleteResponse.ok) {
                  console.log(`?£å Event ${id} deleted from backend`);
                } else {
                  console.warn(`?ú???ï Failed to delete event ${id} from backend:`, deleteResponse.status);
                }
              }
            } catch (error) {
              console.warn('?ú???ï Error deleting event from backend:', error);
            }
            ...
            try {
              ...
              if (stored) {
                ...
                if (parsed.state && parsed.state.events) {
                  ...
                  console.log(`?£å Removed event ${id} from localStorage`);
                }
              }
            } catch (error) {
              console.warn('?ú???ï Error removing event from localStorage:', error);
            }
            
            console.log(`?£å Deleted ${eventsToDelete.length} event(s) with ID ${id}`);
          } else {
            console.warn(`?ú???ï No event found with ID ${id} to delete`);
            set({ isLoading: false });
          }
        } catch (error) {
          console.error('?¥ì Error deleting event:', error);
          set({ error: '???ò?ù?ð?ô ?ñ???÷?ù???¬ ?ô?ð?ù?¿?õ??', isLoading: false });
        }
      },

      setCurrentEvent: (event) => {
        console.log('?ƒôí setCurrentEvent called with:', event?.id);
        set({ currentEvent: event });
      },

      addGuest: async (eventId, guestData) => {
        console.log('?ƒôí addGuest called with:', { eventId, guestData });
        set({ isLoading: true, error: null });
        try {
          ...
          console.log('?ƒôí Generated new guest:', newGuest);
          
          set(state => {
            console.log('?ƒôí Current state events count:', state.events.length);
            console.log('?ƒôí Current event ID:', state.currentEvent?.id);
            ...
            console.log('?ƒôí Before update - currentEvent guests count:', state.currentEvent?.guests?.length);
            console.log('?ƒôí After update - currentEvent guests count:', updatedCurrentEvent?.guests?.length);
            
            console.log('?ƒôí Updated events count:', updatedEvents.length);
            console.log('?ƒôí Updated current event guests count:', updatedCurrentEvent?.guests?.length);
          ...
          if (updatedEvent) {
            syncEventToAPI(updatedEvent).catch(err => {
              console.error('?¥ì Final sync attempt failed:', err);
            });
          }
          
          console.log('?£å addGuest completed successfully');
        } catch (error) {
          console.error('?¥ì Error in addGuest:', error);
          set({ error: '???ò?ù?ð?ô ?ñ?ô?õ?????¬ ???õ?ö???ƒ', isLoading: false });
        }
      },
      ...
      updateGuest: async (eventId, guestId, updates) => {
        ...
        try {
          ...
          if (updatedEvent) {
            ...
            if (!updatedGuest) {
              console.warn('?ú???ï Guest not found in updated event, skipping API sync');
              return;
            }
            ...
            const syncToAPI = async (retries = 3): Promise<void> => {
              try {
                console.log('?ƒìð Syncing guest update to API (minimal payload)...');
                ...
                if (!guestUpdatePayload.phoneNumber || !guestUpdatePayload.guestId || !guestUpdatePayload.eventId) {
                  console.error('?¥ì Invalid guestUpdatePayload - missing required fields:', {
                ...
                console.log('?ƒó? Sending guest update only (not full event):', {
                  eventId: updatedEvent.id,
                  guestId: guestId,
                ...
                if (response.ok) {
                  ...
                  console.log('?£å Guest update synced to API successfully:', {
                    eventId: updatedEvent.id,
                    guestId: guestId,
                  ...
                  if (updates.guestCount !== undefined || updates.source === 'manual_update') {
                    try {
                      console.log('?ƒôä Also updating event directly in server via /api/events/:eventId/guests for persistence...');
                      ...
                      if (directResponse.ok) {
                        ...
                        console.log('?£å Event updated directly in server:', {
                          success: true,
                        ...
                        console.warn('?ú???ï Direct update to /api/events/:eventId/guests failed:', directResponse.status, errorText);
                      }
                    } catch (directError) {
                      console.warn('?ú???ï Failed to update event directly in server:', directError);
                    }
                  }
                } else {
                  const errorText = await response.text();
                  console.error('?¥ì Failed to sync guest update to API:', {
                ...
                console.warn('?ú???ï Failed to sync guest update to API:', error);
                if (retries > 0) {
                  console.log(`?ƒôä Retrying sync (${retries} retries left)...`);
                  await new Promise(resolve => setTimeout(resolve, 1000));
            ...
            if (userStorage) {
              try {
                ...
                if (userId) {
                  ...
                  console.log(`?ƒ÷ñ??ï Invalidated cache for user ${userId} (manual guest update)`);
                }
              } catch (e) {
                // Ignore parsing errors
              }
          ...
          console.error('?¥ì Error in updateGuest:', error);
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ???õ?ö???ƒ', isLoading: false });
        }
      },

      updateGuestResponse: async (eventId, guestId, updatedGuest) => {
        console.log('?ƒúà updateGuestResponse CALLED:', {
          eventId,
          guestId,
          updatedGuest: {
            rsvpStatus: updatedGuest.rsvpStatus,
        ...
        try {
          const currentState = get();
          console.log(`?ƒôä updateGuestResponse called:`, {
            eventId,
            guestId,
          ...
          if (updatedGuest.source === 'guest_link' && updatedGuest.responseDate) {
            ...
            if (updateDate.getTime() < currentDate.getTime()) {
              ...
              console.log(`?ƒôä Updated responseDate to current time for guest_link update`);
            }
          }
          ...
          if (!existingEvent) {
            console.warn(`?ú???ï Event ${eventId} not found in store - loading from API and updating via backend`);
            ...
            if (!fullEvent) {
              console.error(`?¥ì Event ${eventId} not found in API`);
              set({ isLoading: false, error: '?ð?ù?¿?õ?? ?£?ð ???????ð' });
              return;
            }
                ...
                g.id === guestId ? {
                  ...
                  guestCount: (() => {
                    ...
                    if (isUpdateFromGuestLink && updatedGuest.guestCount !== undefined) {
                      ...
                      console.log(`?£å Applying guest_link guestCount update (API path): ${updatedGuest.guestCount} (overriding current: ${g.guestCount})`);
                    ...
                    if (isCurrentFromManual && updatedGuest.guestCount !== undefined && updatedGuest.guestCount !== g.guestCount) {
                      ...
                      if (!isUpdateFromManual || currentDate >= updateDate) {
                        console.log(`?ƒ¢???ï Preserving manual guestCount ${g.guestCount} (update ${updatedGuest.guestCount} would revert it)`);
            ...
            console.log(`?ƒó? Sending full updated event to backend:`, {
            ...
            console.log(`?£å Event updated successfully via backend:`, result);
          ...
          set(state => {
            ...
            const isProblematicGuest = (guest?.firstName?.includes('?ó?õ?¿?õ?ƒ') && guest?.lastName?.includes('???õ?????ù')) ||
                                      (guest?.firstName?.includes('???ð?õ?¿') && guest?.lastName?.includes('?¿?õ?????õ'));
            
            console.log(`?ƒóë Before update - Event found: ${!!event}, Guest found: ${!!guest}`);
            console.log(`?ƒóë Events in store: ${state.events.length}`);
            console.log(`?ƒóë Guest status:`, guest?.rsvpStatus);
            console.log(`?ƒóë Guest count before update:`, guest?.guestCount);
            console.log(`?ƒóë Updated guest count:`, updatedGuest.guestCount);
            console.log(`?ƒóë Updated guest source:`, updatedGuest.source);
            console.log(`?ƒóë Is problematic guest:`, isProblematicGuest);
            
            if (isProblematicGuest) {
              console.log(`?ƒôí PROBLEMATIC GUEST UPDATE IN STORE: ${guest?.firstName} ${guest?.lastName}`, {
                id: guestId,
                eventId,
                    ...
                    if (guest.id === guestId) {
                      ...
                      if (isCurrentFromManual && updatedGuest.guestCount !== undefined && updatedGuest.guestCount !== guest.guestCount && !isGuestLinkUpdate) {
                        ...
                        if (!isNewUpdateFromManual && !isSignificantlyNewer) {
                          console.log(`?ƒ¢???ï Blocking update: current manual guestCount ${guest.guestCount} would be reverted to ${updatedGuest.guestCount} by non-manual or too-recent update`);
                          shouldApplyUpdate = false; // Don't apply this update - it would revert manual change
                        }
                      }
                      
                      if (shouldApplyUpdate && !isNewerUpdate) {
                        console.log(`?ƒôä Applying ${updatedGuest.source} update even though timestamp is older - user-initiated update must be applied`);
                      }
                      ...
                      if (shouldApplyUpdate) {
                        ...
                        if (isManualUpdateEcho) {
                          ...
                          if (isUpdateFromGuestLink && updatedGuest.guestCount !== undefined) {
                            // Guest link update - always use the new value (direct user input)
                            console.log(`?£å Applying guest_link guestCount update: ${updatedGuest.guestCount} (overriding current: ${guest.guestCount})`);
                            ...
                            if (!isBackendFromManual || currentResponseDate >= updateResponseDate) {
                              ...
                              console.log(`?ƒ¢???ï Preserving manual guestCount ${guest.guestCount} (backend update ${updatedGuest.guestCount} would revert it)`);
                        ...
                        console.log(`?ƒô? Merging guest (latest update wins - COMPLETELY REPLACING old values):`, {
                          old: { 
                        ...
                        if (isManualUpdateEcho || isGuestLinkUpdate || isWhatsAppUpdate) {
                          ...
                          console.log(`?ƒôä Applying ${updatedGuest.source} update even though old update is newer - user-initiated update must be applied`);
                        ...
                        console.log(`?ï¡??ï Keeping old guest values (old update is newer and not user-initiated):`, {
                          old: { 
                            rsvpStatus: guest.rsvpStatus, 
                            guestCount: guest.guestCount,
                            responseDate: oldResponseDate.toISOString()
                        ...
                        if (isGuestLinkUpdate) {
                          console.log(`?£å Applying guest_link update even though old update is newer - direct user input must be applied`);
                          const mergedGuest = { 
                            ...guest,
                            ...updatedGuest,
            ...
            if (state.currentEvent?.id === eventId) {
              ...
              updatedCurrentEvent = {
                ...
                guests: state.currentEvent.guests.map(guest => {
                  if (guest.id === guestId) {
                    ...
                    if (isUpdateFromGuestLink && updatedGuest.guestCount !== undefined) {
                      // Guest link update - always use the new value (direct user input)
                      console.log(`?£å Applying guest_link guestCount update in currentEvent: ${updatedGuest.guestCount} (overriding current: ${guest.guestCount})`);
                      finalGuestCount = updatedGuest.guestCount;
                    } else if (isCurrentFromManual && updatedGuest.guestCount !== undefined && updatedGuest.guestCount !== guest.guestCount) {
                      ...
                      if (!isUpdateFromManual || oldResponseDate.getTime() >= newResponseDate.getTime()) {
                        console.log(`?ƒ¢???ï Preserving manual guestCount ${guest.guestCount} in currentEvent (update ${updatedGuest.guestCount} would revert it)`);
                    ...
                    console.log(`?ƒôä Updating guest in currentEvent:`, {
                      guestId,
              ...
              console.log('?ƒôä Updated existing currentEvent for event:', eventId, 'guests:', updatedCurrentEvent.guests.length, 'updatedAt:', newUpdatedAt.toISOString());
            } else {
              ...
              console.log('?ä???ï Update for event', eventId, 'but user is viewing event', state.currentEvent?.id || 'none', '- EventManagement will handle update');
            }
            ...
            console.log(`?£å STORE: After updateGuestResponse - Guest ID: ${guestId}`);
            console.log(`?£å STORE: After updateGuestResponse - Guest name: ${verifyGuest?.firstName} ${verifyGuest?.lastName}`);
            console.log(`?£å STORE: After updateGuestResponse - Guest status:`, verifyGuest?.rsvpStatus);
            console.log(`?£å STORE: After updateGuestResponse - Guest count in events array:`, verifyGuest?.guestCount);
            console.log(`?£å STORE: After updateGuestResponse - Guest source in events array:`, (verifyGuest as any)?.source);
            console.log(`?£å STORE: After updateGuestResponse - Guest count in currentEvent:`, verifyCurrentEventGuest?.guestCount);
            console.log(`?£å STORE: After updateGuestResponse - Guest status in currentEvent:`, verifyCurrentEventGuest?.rsvpStatus);
            console.log(`?£å STORE: After updateGuestResponse - Guest source in currentEvent:`, (verifyCurrentEventGuest as any)?.source);
            console.log(`?£å STORE: Updated currentEvent:`, updatedCurrentEvent?.id, 'guests:', updatedCurrentEvent?.guests?.length);
            console.log(`?£å STORE: Event updatedAt:`, updatedEvent?.updatedAt);
            console.log(`?£å STORE: CurrentEvent updatedAt:`, updatedCurrentEvent?.updatedAt);
            
            // CRITICAL: Check if update was applied correctly
            if (verifyGuest && verifyGuest.rsvpStatus !== updatedGuest.rsvpStatus) {
              console.error(`?¥ì MISMATCH: Guest status in events array (${verifyGuest.rsvpStatus}) doesn't match update (${updatedGuest.rsvpStatus})`);
            }
            if (verifyCurrentEventGuest && verifyCurrentEventGuest.rsvpStatus !== updatedGuest.rsvpStatus) {
              console.error(`?¥ì MISMATCH: Guest status in currentEvent (${verifyCurrentEventGuest.rsvpStatus}) doesn't match update (${updatedGuest.rsvpStatus})`);
            }
            ...
            const finalUpdatedEvents = updatedEvents.map(e => {
              if (e.id === eventId) {
                ...
                const newUpdatedAt = new Date();
                console.log(`?ƒôä Updating event ${eventId} updatedAt to force eventsHash change:`, newUpdatedAt.toISOString());
                return {
                  ...e,
            ...
            console.log(`?ƒôä Creating new events array reference to force React re-render`);
            console.log(`?ƒôä Updated event ${eventId} updatedAt to:`, new Date().toISOString());
            return {
          ...
          if (userStorage) {
            try {
              ...
              if (userId) {
                const cacheKey = CACHE_KEYS.EVENTS(userId);
                cacheService.invalidate(cacheKey);
                console.log(`?ƒ÷ñ??ï Invalidated cache for user ${userId} (guest response update)`);
              }
            } catch (e) {
              // Ignore parsing errors
            }
          ...
          if (updatedEvent) {
            ...
            if (!updatedGuest) {
              console.warn('?ú???ï Updated guest not found, cannot sync to API');
              return updatedEvent;
            }
            
            try {
              console.log('?ƒìð Syncing guest response update to API...');
              console.log('?ƒó? Sending guest update only (not full event to avoid 413):', {
                eventId: updatedEvent.id,
                guestId: guestId,
                updatedGuest: {
                  id: updatedGuest.id,
              ...
              console.log(`?ƒó? Syncing guest update to backend:`, {
              ...
              if (updatedGuest.guestCount !== undefined && updatedGuest.guestCount !== null) {
                console.log(`?£å Guest count included in sync payload: ${updatedGuest.guestCount}`);
              }
              ...
              console.log('?£å Guest update added to batch queue (will be sent shortly)');
                console.log('?£å Update will be processed by webhook service and synced to all devices');
              ...
              try {
                const isProblematicGuest = (updatedGuest.firstName?.includes('?ó?õ?¿?õ?ƒ') && updatedGuest.lastName?.includes('???õ?????ù')) ||
                                          (updatedGuest.firstName?.includes('???ð?õ?¿') && updatedGuest.lastName?.includes('?¿?õ?????õ')) ||
                                          (updatedGuest.firstName?.includes('???ù?ó?õ') && updatedGuest.lastName?.includes('?ó???ƒ'));
                
                console.log('?ƒôä Updating event directly in server via /api/events/:eventId/guests...');
                console.log('?ƒó? Sending updated guest to server:', {
                  eventId: updatedEvent.id,
                  guestId: updatedGuest.id,
                  rsvpStatus: updatedGuest.rsvpStatus,
                  guestCount: updatedGuest.guestCount,
                ...
                if (isProblematicGuest) {
                  console.log(`?ƒôí PROBLEMATIC GUEST SENDING TO SERVER: ${updatedGuest.firstName} ${updatedGuest.lastName}`, {
                    id: updatedGuest.id,
                    eventId: updatedEvent.id,
                    rsvpStatus: updatedGuest.rsvpStatus,
                    guestCount: updatedGuest.guestCount,
                ...
                console.log('?ƒó? Sending complete guest object to server:', {
                ...
                if (apiUpdateResponse.ok) {
                  ...
                  const isProblematicGuest = (guestForServer.firstName?.includes('?ó?õ?¿?õ?ƒ') && guestForServer.lastName?.includes('???õ?????ù')) ||
                                            (guestForServer.firstName?.includes('???ð?õ?¿') && guestForServer.lastName?.includes('?¿?õ?????õ')) ||
                                            (guestForServer.firstName?.includes('???ù?ó?õ') && guestForServer.lastName?.includes('?ó???ƒ'));
                  ...
                  if (isProblematicGuest) {
                    console.log(`?ƒôí PROBLEMATIC GUEST SENT TO SERVER SUCCESSFULLY: ${guestForServer.firstName} ${guestForServer.lastName}`, {
                      id: guestForServer.id,
                      rsvpStatus: guestForServer.rsvpStatus,
                      guestCount: guestForServer.guestCount,
                      source: guestForServer.source,
                  ...
                  console.log(`?£å VERIFIED: Guest ${guestForServer.id} (${guestForServer.firstName} ${guestForServer.lastName}) update sent to server:`, {
                    rsvpStatus: guestForServer.rsvpStatus,
                    guestCount: guestForServer.guestCount,
                    source: guestForServer.source,
                    responseDate: guestForServer.responseDate
                  ...
                  console.log('?£å Server update confirmed - local update preserved in store');
                } else {
                  ...
                  console.error('?¥ì FAILED to update event directly in server:', {
                  ...
                  if (isProblematicGuest) {
                    console.error(`?ƒôí PROBLEMATIC GUEST FAILED TO SEND TO SERVER: ${guestForServer.firstName} ${guestForServer.lastName}`, {
                      id: guestForServer.id,
                      status: apiUpdateResponse.status,
                      error: apiErrorText
                    });
                  ...
                  console.log('?ú???ï Falling back to pendingUpdates mechanism');
                }
              } catch (apiError) {
                console.warn('?ú???ï Error updating event directly in server:', apiError);
                // Fallback: Still add to pendingUpdates for webhook service to process
                console.log('?ú???ï Falling back to pendingUpdates mechanism');
              }
            } catch (error) {
              console.warn('?ú???ï Failed to sync guest response update to API (will use localStorage):', error);
              // Don't retry with full event - it will fail with 413 for large events
            }
          }
        } catch (error) {
          console.error('?¥ì Error in updateGuestResponse:', error);
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ?¬?ò?õ?ñ?¬ ???õ?ö???ƒ', isLoading: false });
        }
      },
      ...
      deleteGuest: async (eventId, guestId) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???÷?ù???¬ ???õ?ö???ƒ', isLoading: false });
        }
      },
      ...
      importGuestsFromExcel: async (eventId, data) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ?ù?ù?ñ?õ?ð ???¬?õ???ù?¥', isLoading: false });
        }
      },
      ...
      exportGuestsToExcel: async (eventId) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ?ù?ù???õ?ð ???¬?õ???ù?¥', isLoading: false });
        }
      },
      ...
      createCampaign: async (campaignData) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ?ù???ù?¿?¬ ?ô???????ù?ù?ƒ', isLoading: false });
        }
      },
      ...
      scheduleCampaign: async (eventId: string, campaignId: string, scheduledDate: Date): Promise<void> => {
        ...
        try {
          ...
          schedulerService.scheduleCampaign(updatedCampaign, async () => {
            try {
              console.log(`?ï? Scheduled time reached for campaign: ${updatedCampaign.name}`);
          ...
          console.log(`?£å Scheduled campaign: ${updatedCampaign.name} for ${scheduledDate.toLocaleString('he-IL')}`);
          set({ isLoading: false });
        } catch (error) {
          console.error('Error scheduling campaign:', error);
          set({ error: '???ò?ù?ð?ô ?ñ?¬?ö???õ?ƒ ?ô???????ù?ù?ƒ', isLoading: false });
          throw error;
        }
      },
      ...
      sendCampaign: async (eventId: string, campaignId: string): Promise<BulkMessageResult> => {
        ...
        console.log('?ƒó? System is now actively waiting for guest responses via WhatsApp buttons and guest links...');
        set({ isLoading: true, error: null });
        try {
          const event = get().events.find(e => e.id === eventId);
          if (!event) {
          ...
          const isEventDayReminder = campaign.name === '?¬?ö?¢?õ?¿?¬ ?ù?õ?¥ ?ô?ð?ù?¿?õ??';
          
          // CRITICAL: Filter guests based on campaign type
          // 1. Some campaigns (?ô?ö?????ô ?¿?ð???õ???ù?¬, ?¬?ö?¢?õ?¿?¬ ?????ù?ù?ô, ?¬?ö?¢?õ?¿?¬ ???ñ?õ???ù?¬) should only be sent to guests with status 'pending' (?£?ð ?????ô) or 'maybe' (?ð?õ?£?ù ???ò?ù??)
          // 2. Other campaigns (?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô, ?ô?õ?ó???¬ ?¬?õ?ó?ô ?£???ò?ù???ù?¥, ?¬?ö?¢?õ?¿?¬ ?ù?õ?¥ ?ô?ð?ù?¿?õ??) should only be sent to guests who confirmed (???ò?ù??)
          // 3. Custom campaigns (default) should only be sent to guests with status 'pending' or 'maybe' (?£?ð ?????ô ?õ???¬?£?ñ?ø)
          const campaignsForNonResponded = ['?ô?ö?????ô ?¿?ð???õ???ù?¬', '?¬?ö?¢?õ?¿?¬ ?????ù?ù?ô', '?¬?ö?¢?õ?¿?¬ ???ñ?õ???ù?¬'];
          const campaignsForConfirmed = ['?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô', '?ô?õ?ó???¬ ?¬?õ?ó?ô ?£???ò?ù???ù?¥', '?¬?ö?¢?õ?¿?¬ ?ù?õ?¥ ?ô?ð?ù?¿?õ??'];
          
          let filteredGuests;
          if (campaignsForNonResponded.includes(campaign.name)) {
            // Only send to guests with status 'pending' (?£?ð ?????ô) or 'maybe' (?ð?õ?£?ù ???ò?ù??)
            filteredGuests = guests.filter(guest => 
              guest.rsvpStatus === 'pending' || guest.rsvpStatus === 'maybe'
            );
            console.log(`?ƒóê Campaign "${campaign.name}": ${filteredGuests.length} of ${guests.length} guests will receive the message (filtered: only guests with status 'pending' or 'maybe')`);
            console.log(`?£å Filtering: Only sending to guests with status === 'pending' (?£?ð ?????ô) or 'maybe' (?ð?õ?£?ù ???ò?ù??)`);
          } else if (campaignsForConfirmed.includes(campaign.name)) {
            // Only send to guests who confirmed (???ò?ù??)
            filteredGuests = guests.filter(guest => guest.rsvpStatus === 'confirmed');
            console.log(`?ƒóê Campaign "${campaign.name}": ${filteredGuests.length} of ${guests.length} guests will receive the message (filtered: only guests with status 'confirmed')`);
            console.log(`?£å Filtering: Only sending to guests with status === 'confirmed' (???ò?ù??)`);
          } else {
            // Default: send only to guests with status 'pending' or 'maybe' (?£?ð ?????ô ?õ???¬?£?ñ?ø)
            // This ensures custom campaigns only target guests who haven't confirmed or declined
            filteredGuests = guests.filter(guest => 
              guest.rsvpStatus === 'pending' || guest.rsvpStatus === 'maybe'
            );
            console.log(`?ƒóê Campaign "${campaign.name}": ${filteredGuests.length} of ${guests.length} guests will receive the message (filtered: only guests with status 'pending' or 'maybe')`);
            console.log(`?£å Filtering: Only sending to guests with status === 'pending' (?£?ð ?????ô) or 'maybe' (???¬?£?ñ?ø)`);
          }
          ...
          // This ensures "?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô" always uses "today" template, even if campaign has different templateName
          ...
          if (campaign.name === '?ô?ö?????ô ?¿?ð???õ???ù?¬') {
            templateNameForCampaign = 'aa'; // Template name in Meta Business Manager
          } else if (campaign.name === '?¬?ö?¢?õ?¿?¬ ?????ù?ù?ô') {
            // Use template 'aa' for "?¬?ö?¢?õ?¿?¬ ?????ù?ù?ô"
            templateNameForCampaign = 'aa';
          } else if (campaign.name === '?¬?ö?¢?õ?¿?¬ ???ñ?õ???ù?¬') {
            templateNameForCampaign = 'aa';
          } else if (campaign.name === '?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô') {
            // CRITICAL: Always use template 'today' for "?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô" campaign
            templateNameForCampaign = 'today';
            console.log('?ƒóë Campaign "?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô" - using template "today"');
          } else if (campaign.name === '?¬?ö?¢?õ?¿?¬ ?ù?õ?¥ ?ô?ð?ù?¿?õ??') {
            // CRITICAL: Don't use template for "?¬?ö?¢?õ?¿?¬ ?ù?õ?¥ ?ô?ð?ù?¿?õ??" - send custom message content instead
            // User wants to send the campaign message content, not a template placeholder
            templateNameForCampaign = undefined;
            console.log('?ƒóë Campaign "?¬?ö?¢?õ?¿?¬ ?ù?õ?¥ ?ô?ð?ù?¿?õ??" - will send custom message content, not template');
          ...
          const personalizedMessages = await Promise.all(filteredGuests.map(async (guest) => {
            ...
            console.log('?ƒô÷ Campaign - Guest ID:', guest.id, 'for guest:', `${guest.firstName} ${guest.lastName}`);
            console.log('?ƒô÷ Campaign - Original message:', personalizedMessage);
            
            // Find the table number for this guest
            const guestTable = event.tables?.find(table => table.guests.includes(guest.id));
            const tableNumber = guestTable ? guestTable.number : '?£?ð ?ô?õ?????ô';
            ...
            const coupleName = event.coupleName || 
              (event.groomName && event.brideName ? `${event.groomName} & ${event.brideName}` : 
               event.groomName || event.brideName || '?ô?ö?õ?ò');
            ...
            personalizedMessage = personalizedMessage
              ...
              .replace(/\{\{event_type\}\}/g, event.eventTypeHebrew || '?÷?¬?õ???ô')
            ...
            console.log('?ƒô÷ Campaign - Final message:', personalizedMessage);
            console.log('?ƒóï Message length:', personalizedMessage.length, 'characters');
            console.log('?ƒóï SMS Message length:', personalizedSmsMessage.length, 'characters');
            ...
            if (isEventDayReminder) {
              try {
                ...
                console.log('?ƒó? Generated QR code for guest:', guest.id, qrCodeImageUrl);
              } catch (error) {
                console.error('?¥ì Error generating QR code:', error);
              }
            }
          ...
          const recipients: MessageRecipient[] = personalizedMessages.map(({ guest, message, smsMessage, qrCodeImageUrl }) => {
            const guestTable = event.tables?.find(table => table.guests.includes(guest.id));
            const tableNumber = guestTable ? guestTable.number?.toString() : '?£?ð ?ô?õ?????ô';
            // CRITICAL: Find the original row number of the guest in the event (not filtered)
            const originalRowNumber = event.guests.findIndex(g => g.id === guest.id) + 1;
            // Use helper function to ensure production URL (works on all devices)
            const guestLink = generateGuestResponseLink(eventId, guest.id, guest.firstName, guest.lastName, guest.phoneNumber, originalRowNumber);
            ...
            if (templateNameForCampaign === 'aa' || templateNameForCampaign === 'AA') {
              ...
              templateParams = {
                ...
                event_type: event.eventTypeHebrew || '?÷?¬?õ???ô',
                ...
                guest_response_link: guestLink, // CRITICAL: Required for template "aa" URL button at index 0 ("?£???ó?¢?õ?ƒ ???ø?ø?õ?? ?ô?ò???ô")
                language: 'he'
              };
              
              // DEBUG: Log template parameters
              console.log(`?ƒôí DEBUG Template Parameters for "${templateNameForCampaign}" (sendCampaign):`, {
              ...
              templateParams = {
                ...
                event_type: event.eventTypeHebrew || '?÷?¬?õ???ô', // Parameter 2
            ...
            console.log('?ƒó¥ DEBUG: Creating text-only message (no buttons)');
            console.log('?ƒó¥ DEBUG: Guest link:', guestLink);
            
            // Add response text to message if not already present
            // Add at the end of the message
            const responseText = `\n\n?£?ô???ù?ñ:\n?à? ???ò?ù?? - ${guestLink}\n?à? ?£?ð ?ð?õ?¢?£ ?£?ô?ò?ù?? - ${guestLink}?status=declined`;
            
            // Check if message already contains response instructions
            const hasResponseText = message.includes('?£?ô???ù?ñ') || message.includes('???ò?ù??') || message.includes('?£?ð ?ð?õ?¢?£ ?£?ô?ò?ù??');
            const finalMessage = hasResponseText ? message : message + responseText;
            
            console.log('?ƒó¥ DEBUG: Final message (with response text):', finalMessage.substring(0, 200) + '...');
            
            const eventInvitationImageUrl = event.invitationImageUrl || qrCodeImageUrl || campaign.imageUrl;
            console.log('?ƒö???ï sendCampaign - Image URL priority:', {
            ...
            console.log('?ƒó¥ DEBUG: Recipient created WITHOUT buttons (text-only)');
            console.log('?ƒó¥ DEBUG: Recipient channel:', guest.channel);
          });
          
          console.log('?ƒó¥ DEBUG: Total recipients created:', recipients.length);
          console.log('?ƒó¥ DEBUG: All recipients are text-only (no buttons)');
          ...
          console.log('?ƒö???ï Image URL priority check:', {
            eventInvitationImageUrl: event.invitationImageUrl,
            campaignImageUrl: campaign.imageUrl,
          ...
          console.log('?ƒôø DEBUG: ========== BEFORE SEND BULK MESSAGES ==========');
          console.log('?ƒôø DEBUG: Recipients count:', recipients.length);
          console.log('?ƒôø DEBUG: Recipients with buttons:', recipients.filter(r => r.buttons && r.buttons.length > 0).length);
          recipients.forEach((r, idx) => {
            console.log(`?ƒôø DEBUG: Recipient ${idx}:`, {
          ...
          console.log(`?ƒó? Campaign sent successfully! ${result.successful} messages sent, ${result.failed} failed`);
          console.log(`?ƒñâ System is now actively waiting for guest responses...`);
          // Webhook polling status logged only when needed
          console.log(`?£å Updated messageStatus for ${result.successful} guests to "sent"`);
          ...
          if (updatedEvent) {
            syncEventToAPI(updatedEvent).catch(err => {
              console.warn('?ú???ï Failed to sync updated event to API:', err);
            });
          }

          return result;
        } catch (error) {
          set({ error: '???ò?ù?ð?ô ?ñ???£?ù?÷?¬ ?ô???????ù?ù?ƒ', isLoading: false });
          throw error;
        }
      },
      ...
      resendFailedMessages: async (eventId: string, campaignId: string): Promise<BulkMessageResult> => {
        ...
        console.log('?ƒó? Resending failed messages - System is now actively waiting for guest responses...');
        set({ isLoading: true, error: null });
        try {
          const event = get().events.find(e => e.id === eventId);
          if (!event) {
          ...
          if (failedGuests.length === 0) {
            console.log('?ä???ï No guests with failed messages found for this campaign');
            set({ isLoading: false });
            return {
              totalSent: 0,
              successful: 0,
          ...
          console.log(`?ƒóê Resending campaign "${campaign.name}" to ${failedGuests.length} guests with failed messages`);
          ...
          const personalizedMessages = await Promise.all(failedGuests.map(async (guest) => {
            ...
            // Add response options as text in the message instead of buttons
            const responseText = `\n\n?£?ô???ù?ñ:\n?à? ???ò?ù?? - ${guestLink}\n?à? ?£?ð ?ð?õ?¢?£ ?£?ô?ò?ù?? - ${guestLink}?status=declined`;
            const hasResponseText = personalizedMessage.includes('?£?ô???ù?ñ') || personalizedMessage.includes('???ò?ù??') || personalizedMessage.includes('?£?ð ?ð?õ?¢?£ ?£?ô?ò?ù??');
            ...
            const templateCoupleName = event.coupleName || (event.groomName && event.brideName ? `${event.groomName} & ${event.brideName}` : '?ô?ö?õ?ò');
            const templateGroomName = event.groomName || '';
            const templateBrideName = event.brideName || '';
          ...
          console.log(`?ƒó? Resend completed! ${result.successful} messages sent successfully, ${result.failed} failed`);
          console.log(`?£å Updated messageStatus for ${result.successful} guests from "failed" to "sent"`);
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???£?ù?÷?ô ?÷?õ?ö?¿?¬ ?£?¢???£?õ???õ?¬', isLoading: false });
          throw error;
        }
      },
      ...
      sendTestMessage: async (phoneNumber: string, message: string, channel: 'whatsapp'): Promise<boolean> => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???£?ù?÷?¬ ?ô?õ?ó???¬ ?ñ?ó?ù???ô', isLoading: false });
          return false;
        }
      },
      ...
      createVenueLayout: async (eventId: string, layoutData) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ?ù???ù?¿?¬ ?????ù???¬ ?ð?õ?£?¥', isLoading: false });
        }
      },
      ...
      updateVenueLayout: async (eventId: string, updates) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ?????ù???¬ ?ð?õ?£?¥', isLoading: false });
        }
      },
      ...
      updateTablePosition: async (eventId: string, tableId: string, x: number, y: number) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ???ù???õ?¥ ???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      updateTableSize: async (eventId: string, tableId: string, width: number, height: number) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ?ò?õ?ó?£ ???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      updateTableRotation: async (eventId: string, tableId: string, rotation: number) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ???ù?ñ?õ?ñ ???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      updateTableShape: async (eventId: string, tableId: string, shape: 'rectangle' | 'circle' | 'oval') => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ???õ?¿?¬ ???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      restoreDeletedEvent: async (deletedEventId: string) => {
        ...
        try {
          ...
          console.log(`?ƒôä Attempting to restore event ${deletedEventId} from backend...`);
          ...
          if (restoreResponse.ok) {
            ...
            console.log(`?£å Event restored from backend:`, restoreData.event);
            console.log(`?ƒóê Guests restored: ${restoreData.guestsRestored || restoreData.event.guests?.length || 0}`);
            
            // CRITICAL: Log guest details to verify they were restored
            if (restoreData.event.guests && restoreData.event.guests.length > 0) {
              console.log(`?ƒóë Restored guests with RSVP data:`, restoreData.event.guests.map((g: any) => ({
                id: g.id,
                name: `${g.firstName} ${g.lastName}`,
                phone: g.phoneNumber,
                rsvpStatus: g.rsvpStatus,
            ...
            console.warn(`?ú???ï Backend restore failed, trying local restore...`);
            ...
            if (deletedEvent) {
              ...
              console.log(`?ƒóê Restoring event locally with ${restoredEvent.guests.length} guests`);
              if (restoredEvent.guests.length > 0) {
                console.log(`?ƒóë Guest details:`, restoredEvent.guests.map((g: any) => ({
                  id: g.id,
                  name: `${g.firstName} ${g.lastName}`,
                  phone: g.phoneNumber,
                  rsvpStatus: g.rsvpStatus,
              ...
              console.log(`?£å Event restored locally with ${restoredEvent.guests.length} guests`);
              return true;
            }
            
            const errorData = await restoreResponse.json().catch(() => ({ error: restoreResponse.statusText }));
            console.error(`?¥ì Failed to restore event:`, errorData);
            set({ error: `???ò?ù?ð?ô ?ñ???÷?ö?õ?¿ ?ô?ð?ù?¿?õ??: ${errorData.error || '?ô?ð?ù?¿?õ?? ?£?ð ???????ð'}`, isLoading: false });
            return false;
          }
        } catch (error) {
          console.error('?¥ì Error restoring event:', error);
          set({ error: '???ò?ù?ð?ô ?ñ???÷?ö?õ?¿ ?ô?ð?ù?¿?õ??', isLoading: false });
          return false;
        }
      },
      ...
      permanentlyDeleteEvent: async (deletedEventId: string) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???÷?ù???ô ???õ???ù?¬ ???£ ?ô?ð?ù?¿?õ??', isLoading: false });
          return false;
        }
      },
      ...
      restoreEvents: () => {
        try {
          ...
          if (stored) {
            const parsed = JSON.parse(stored);
            if (parsed.state && parsed.state.events) {
              console.log('?ƒôä Restoring events from localStorage:', parsed.state.events.length);
              console.log('?ƒóë Events data:', parsed.state.events);
              ...
              parsed.state.events.forEach((event: any, index: number) => {
                ...
                if (guestCount > 0) {
                  console.log(`?ƒóë Guests for event ${event.id}:`, event.guests.map((g: any) => ({
                    id: g.id,
                    name: `${g.firstName} ${g.lastName}`,
                    phone: g.phoneNumber,
                    rsvpStatus: g.rsvpStatus,
              ...
              // CRITICAL: Preserve ALL guest data including RSVP status, guest count, notes, and actual attendance
              set((state) => {
                console.log('?ƒôä Current state before restore:', state);
              ...
              console.log(`?£å Restored ${parsed.state.events.length} events from localStorage`);
              const totalGuests = parsed.state.events.reduce((sum: number, e: any) => sum + (e.guests?.length || 0), 0);
              console.log(`?£å Total guests restored: ${totalGuests}`);
          ...
          console.error('?¥ì Error restoring events:', error);
          return false;
        }
      },
      ...
      forceRefresh: () => {
        try {
          ...
          if (stored) {
            const parsed = JSON.parse(stored);
            console.log('?ƒôä Force refreshing from localStorage...', parsed);
          ...
          console.error('?¥ì Error force refreshing:', error);
          return false;
        }
      },
      ...
      cleanupLocalStorage: () => {
        try {
          ...
          if (stored) {
            const parsed = JSON.parse(stored);
            console.log('?ƒ?? Cleaning up localStorage...', parsed);
            ...
            if (parsed.state && parsed.state.events) {
              ...
              if (uniqueEvents.length !== parsed.state.events.length) {
                console.log(`?ƒ?? Removed ${parsed.state.events.length - uniqueEvents.length} duplicate events`);
          ...
          console.error('?¥ì Error cleaning up localStorage:', error);
          return false;
        }
      },
      ...
      updateExistingEventsCampaigns: () => {
        console.log('?ƒôä Updating existing events campaigns with consistent variable names...');
        set(state => {
          const updatedEvents = state.events.map(event => {
            ...
            const updatedCampaigns = event.campaigns.map(campaign => {
              console.log(`?£å Updating campaign "${campaign.name}" for event "${event.coupleName}"`);
              ...
              let updatedMessage = campaign.message
                ...
                .replace(/?ñ?ñ?¿?¢?ô,\s*\{\{groom_name\}\} ?õ\{\{bride_name\}\}/g, '?ñ?ñ?¿?¢?ô,\n{{couple_name}} ?ƒòõ')
                .replace(/?ñ?ñ?¿?¢?ô,\s*\{\{groom_name\}\} ?õ\{\{bride_name\}\}\s*?ƒòõ/g, '?ñ?ñ?¿?¢?ô,\n{{couple_name}} ?ƒòõ');
              
              // Special handling for "?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô" campaign - remove guest_response_link and use first_name
              if (campaign.name === '?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô') {
                updatedMessage = updatedMessage
                  .replace(/\{\{guest_name\}\}/g, '{{first_name}}')
                  .replace(/?ƒô÷\s*?£???ó?¢?ƒ ???ø?ø?õ??[^:]*:\s*\{\{guest_response_link\}\}/g, '?ƒô÷ ?£???ó?¢?ƒ ???ø?ø?õ?? ?ô?ô?ò???ô ?£?÷??')
                  .replace(/\{\{guest_response_link\}\}/g, '')
                  .replace(/?£???ó?¢?ƒ ???ø?ø?õ??[^:]*:\s*\{\{guest_response_link\}\}/g, '?£???ó?¢?ƒ ???ø?ø?õ?? ?ô?ô?ò???ô ?£?÷??')
                  .replace(/?ñ?ñ?¿?¢?ô,\s*\{\{couple_name\}\}\s*?ƒòõ/g, '')
                  .trim();
              }
              ...
              if (updatedSmsMessage) {
                ...
                // Special handling for "?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô" campaign SMS
                if (campaign.name === '?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô') {
                  updatedSmsMessage = updatedSmsMessage
                    .replace(/\{\{guest_name\}\}/g, '{{first_name}}')
                    .replace(/?ƒô÷\s*?£???ó?¢?ƒ ???ø?ø?õ??[^:]*:\s*\{\{guest_response_link\}\}/g, '?ƒô÷ ?£???ó?¢?ƒ ???ø?ø?õ?? ?ô?ô?ò???ô ?£?÷??')
              ...
              if (campaign.name === '?ô?ö?????ô ?¿?ð???õ???ù?¬' || 
                  campaign.name === '?¬?ö?¢?õ?¿?¬ ?????ù?ù?ô' || 
                  campaign.name === '?¬?ö?¢?õ?¿?¬ ???ñ?õ???ù?¬') {
                // Force lowercase 'aa' - Meta is case-sensitive!
                updatedTemplateName = 'aa'; // Template name in Meta Business Manager (lowercase!)
                console.log(`   ?ƒóë Set templateName to 'aa' (lowercase) for campaign "${campaign.name}"`);
              } else if (campaign.templateName === 'AA') {
                // Fix old campaigns that might have 'AA' instead of 'aa'
                updatedTemplateName = 'aa';
                console.log(`   ?ƒóë Fixed templateName from 'AA' to 'aa' for campaign "${campaign.name}"`);
              } else if (campaign.name === '?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô') {
                // "?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô" campaign uses template "today"
                updatedTemplateName = 'today';
                console.log(`   ?ƒóë Set templateName to 'today' for campaign "${campaign.name}"`);
              }
          ...
          console.log(`?£å Updated ${updatedEvents.length} events`);
      ...
      recreateCampaigns: async (eventId: string) => {
        console.log('?ƒôä recreateCampaigns called with eventId:', eventId);
        let event = get().events.find(e => e.id === eventId);
        
        // If event not found in store, try to fetch from API
        if (!event) {
          console.log('?ú???ï Event not found in store, fetching from API...');
          try {
            const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://localhost:3002';
            ...
            if (response.ok) {
              ...
              if (data.success && data.events) {
                ...
                if (event) {
                  console.log('?£å Found event in API, adding to store...');
            ...
            console.error('?¥ì Error fetching event from API:', apiError);
          }
        }
        
        if (!event) {
          console.log('?¥ì Event not found for campaign recreation');
          throw new Error('?ô?ð?ù?¿?õ?? ?£?ð ???????ð. ?ð???ð ?¿?????ƒ ?ð?¬ ?ô?ó?? ?õ?????ô ???õ?ñ.');
        }

        console.log('?ƒóå Found event:', event.coupleName, 'with', event.campaigns?.length || 0, 'existing campaigns');
        ...
        console.log('?ƒ÷ñ??ï Deleting old campaigns and creating new ones...');
...
?ð???÷???õ ?????÷?ù?¥ ?£?ô?ö???ù?ƒ ?ð?õ?¬?ú ?£{{event_type}} ???£ {{groom_name}} ?õ{{bride_name}}! 

?ƒóå {{event_date}} | ?ƒõð {{event_time}}
?ƒóí {{venue}}
...
{{couple_name}}`,
          ...
          },
          {
            ...
            eventId: eventId,
            name: '?¬?ö?¢?õ?¿?¬ ?????ù?ù?ô',
            message: `???£?õ?¥! ?¿?? ???ñ?õ???ù?ù?¥ ?£?ð?ù?¿?õ??! ?ƒîê

?ƒóå {{event_date}}
?ƒóí {{venue}}

?ð?¥ ???ó?ù?ù?ƒ ?£?ð ?ð?ù???¿?¬?¥ ?ô?ò???ô, ?ð???ð ?????õ ?ö?ð?¬ ???¢???ù?õ!

?ƒô÷ ?£?ð???¿ ?ô?ò???ô ?õ?£???ó?¢?ƒ ???ø?ø?õ??: {{guest_response_link}}
...
?¬?ö?¢?õ?¿?¬: ?ô{{event_type}} ???£ {{couple_name}} ???¬???¿?ñ! 

?ƒóå ?¬?ð?¿?ù?ú: {{event_date}}
?ƒõð ?????ô: {{event_time}}
?ƒóí ???ù???õ?¥: {{venue}}

?ð?¥ ???ó?ù?ù?ƒ ?£?ð ?ð???¿?¬ ?ô?ò???ô, ?ð???ð ?????ô ?ö?ð?¬ ?ñ???ù???õ?¿:
{{guest_response_link}}
...
???÷?¢?ù?¥ ?£?¿?ð?õ?¬ ?ð?õ?¬?ú!`,
          ...
          },
          {
            ...
            eventId: eventId,
            name: '?¬?ö?¢?õ?¿?¬ ???ñ?õ???ù?¬',
            message: `?ï? ???£?õ?¥ {{guest_name}}!

?¬?ö?¢?õ?¿?¬ ?ð?÷?¿?õ???ô: ?ð?¬?¥ ???õ?ö?????ù?¥ ?ð?£ ?ô{{event_type}} ???£ {{couple_name}}  ?ô?ð?ù?¿?õ?? ?????? ?ñ???¿?õ?ñ ?ð???ù ?ð???¿?õ ?ô?ò???¬?¢?¥
...
?ð???ð ?ð???¿/?ù ?ô?ò???ô ???ó ???õ?? ?ô???ñ?õ??:

?ƒô÷ {{guest_response_link}}
...
{{couple_name}}`,
        ...
        set(state => {
          ...
          if (eventIndex < 0) {
            console.error('?¥ì CRITICAL: Event not found in store after adding!');
            throw new Error('?ô?ð?ù?¿?õ?? ?£?ð ???????ð ?ñ???ð?ò?¿ ?ô???¬?õ???ù?¥');
          }
          
          const existingEvent = state.events[eventIndex];
          console.log('?ƒóê Event before update:', {
            id: existingEvent.id,
          ...
          if (!updatedEvent) {
            console.error('?¥ì CRITICAL: Event disappeared after update!');
            throw new Error('?ô?ð?ù?¿?õ?? ?????£?¥ ?£?ð?÷?¿ ?ô???ó?¢?õ?ƒ - ?ö?ô ?£?ð ?ð???õ?¿ ?£???¿?õ?¬!');
          }
          
          console.log('?ƒôä Updated events in state');
          console.log('?ƒóê Event after update:', {
            id: updatedEvent.id,
          ...
          if (updatedEvent.guests?.length !== existingEvent.guests?.length) {
            console.error('?¥ì CRITICAL: Guest count changed during campaign update!', {
              before: existingEvent.guests?.length || 0,
              after: updatedEvent.guests?.length || 0
            });
            throw new Error('?ð?õ?ñ?ó?ƒ ???¬?õ???ù ?ð?õ?¿?÷?ù?¥ ?ñ???ô?£?ú ???ó?¢?õ?ƒ ???????ù?ù???ù?¥!');
          }
        ...
        if (updatedEvent) {
          try {
            await syncEventToAPI(updatedEvent);
            console.log('?£å Recreated campaigns synced to API successfully');
          } catch (error) {
            console.error('?¥ì Failed to sync recreated campaigns to API:', error);
            // Don't throw - the campaigns were created locally, API sync is secondary
          }
        }

        console.log('?ƒôä Recreated campaigns with correct guest links');
        console.log('?ƒóê New campaigns created:', newCampaigns.length);
        console.log('?ƒô÷ Sample link from first campaign:', newCampaigns[0]?.message?.includes('?guest={{guest_id}}') ? 'CORRECT' : 'INCORRECT');
        console.log('?ƒô÷ Full message preview:', newCampaigns[0]?.message?.substring(0, 200) + '...');
      },
      ...
      addTable: async (eventId: string, tableData: Omit<Table, 'id' | 'createdAt' | 'updatedAt'>) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ?ô?õ?????¬ ?ô???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      updateTable: async (eventId: string, tableId: string, updates: Partial<Table>) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???ó?¢?õ?ƒ ?ô???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      deleteTable: async (eventId: string, tableId: string) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ???÷?ù???¬ ?ô???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      assignGuestToTable: async (eventId: string, guestId: string, tableId: string, seatNumber?: number) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ?ô?????ð?¬ ?ô?ð?õ?¿?÷ ?£???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      removeGuestFromTable: async (eventId: string, guestId: string) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ?ô???¿?¬ ?ô?ð?õ?¿?÷ ???ô???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      moveGuestToTable: async (eventId: string, guestId: string, newTableId: string, newSeatNumber?: number) => {
        ...
        try {
          ...
          set({ error: '???ò?ù?ð?ô ?ñ?ô???ñ?¿?¬ ?ô?ð?õ?¿?÷ ?£???õ?£?÷?ƒ', isLoading: false });
        }
      },
      ...
      getAllEvents: () => {
        ...
        if (!isAdmin) {
          throw new Error('?¿?? ?????ô?£ ?ù?¢?õ?£ ?£?¿?ð?õ?¬ ?ð?¬ ?¢?£ ?ô?ð?ù?¿?õ???ù?¥');
        }

        // ???¿?ù?ð?¬ ?¢?£ ?ô?ð?ù?¿?õ???ù?¥ ??-localStorage
        const stored = localStorage.getItem('rsvp-events-storage');
        if (!stored) {
          return [];
        }
      ...
      cleanupOtherUsersEvents: () => {
        try {
          ...
          if (!userId) {
            console.warn('?ú???ï No userId found - cannot clean up events');
            return;
          }

          console.log(`?ƒ?? Starting cleanup for user ${userId}...`);

          // Get all events from localStorage
          const stored = localStorage.getItem('rsvp-events-storage');
          if (!stored) {
            console.log('?ä???ï No events in storage to clean up');
            return;
          }
          ...
          console.log(`?ƒóë Found ${allEvents.length} total events in storage`);
          console.log('?ƒóë Events details:', allEvents.map((e: Event) => ({ 
            id: e.id, 
            userId: e.userId, 
            name: e.coupleName,
          ...
          if (removedEvents.length > 0) {
            ...
            console.log(`?£å Cleaned up ${removedEvents.length} events. Kept ${userEvents.length} events for current user.`);
            console.log(`?£å State updated with ${userEvents.length} events`);
          } else {
            console.log('?ä???ï No events from other users found - nothing to clean up');
          }
        } catch (error) {
          console.error('?¥ì Error cleaning up events:', error);
        }
      },
      ...
      syncCurrentEventToAPI: async (eventId?: string) => {
        try {
          ...
          if (!eventToSync) {
            console.warn('?ú???ï No event to sync:', eventId || 'currentEvent');
            return;
          }
          
          // Only sync if event has guests (to avoid unnecessary syncs)
          if (!eventToSync.guests || eventToSync.guests.length === 0) {
            console.log(`?ï¡??ï Skipping sync for event ${eventToSync.id} - no guests`);
            return;
          }
          
          console.log(`?ƒôä Auto-syncing event ${eventToSync.id} with ${eventToSync.guests.length} guests...`);
          await syncEventToAPI(eventToSync);
          console.log(`?£å Auto-synced event ${eventToSync.id} successfully`);
        } catch (error) {
          console.warn('?ú???ï Failed to auto-sync event:', error);
      ...
      syncAllEventsToAPI: async () => {
        ...
        try {
          ...
          if (!userId) {
            throw new Error('?£?ð ???????ð userId - ?ð???ð ?ô?¬?÷?ñ?¿ ???÷?ó??');
          }

          // Get all events from localStorage
          const stored = localStorage.getItem('rsvp-events-storage');
          if (!stored) {
            throw new Error('?£?ð ???????ð?õ ?ð?ù?¿?õ???ù?¥ ?ñ-localStorage');
          }
          ...
          if (userEvents.length === 0) {
            throw new Error('?£?ð ???????ð?õ ?ð?ù?¿?õ???ù?¥ ?£?????¬???? ?ô???õ?¢?÷?ù');
          }

          console.log(`?ƒôä Syncing ${userEvents.length} events to API for user ${userId}...`);
          ...
          for (const event of userEvents) {
            try {
              ...
              await syncEventToAPI(event);
                syncedCount++;
              console.log(`?£å Synced event "${event.coupleName || `${event.groomName} & ${event.brideName}`}" (${event.id}) to API with ${event.guests?.length || 0} guests`);
            } catch (error: any) {
                failedCount++;
              console.error(`?¥ì Failed to sync event "${event.coupleName || `${event.groomName} & ${event.brideName}`}":`, error);
              // syncEventToAPI already handles 413 errors internally, so we just log the failure
            }
          }

          console.log(`?£å Sync complete: ${syncedCount} synced, ${failedCount} failed`);
          ...
          if (failedCount > 0) {
            throw new Error(`?????¢?¿???õ ${syncedCount} ?ð?ù?¿?õ???ù?¥, ${failedCount} ???¢???£?õ`);
          }
          
          return { synced: syncedCount, failed: failedCount };
        } catch (error) {
          console.error('?¥ì Error syncing all events:', error);
          set({ error: error instanceof Error ? error.message : '???ò?ù?ð?ô ?ñ?????¢?¿?õ?ƒ ?ð?ù?¿?õ???ù?¥', isLoading: false });
          throw error;
        }
      },
    ...
    }),
    {
      ...
      partialize: (state) => {
        ...
        try {
          ...
          console.warn('?ú???ï Could not get userId in partialize:', e);
        }
        ...
        try {
            ...
            if (parsed.state?.events && parsed.state.events.length > 0) {
              ...
              currentEventsFromState.forEach((stateEvent: Event) => {
                if (!mergedEvents.find((e: Event) => e.id === stateEvent.id)) {
                  console.log('?ƒò? Adding new event from state to storage:', stateEvent.id);
                ...
                return {
                  ...
                  guests: event.guests
                    ?.filter((guest: Guest) => {
                      ...
                      if (deletedGuestIds.includes(guest.id)) {
                        console.log(`?ƒú½ Filtering out deleted guest from localStorage: ${guest.firstName} ${guest.lastName} (${guest.id})`);
                        return false;
                      }
                      return true;
                    })
              ...
              console.log(`?ƒò? Saving ${cleanedMergedEvents.length} events to localStorage (userId: ${currentUserId || 'none'})`);
          ...
          if (state.events && state.events.length > 0) {
            console.log('?ƒò? No storage found, saving current state events:', state.events.length);
          ...
          console.error('?¥ì Error in partialize:', error);
        }
          ...
          return {
            ...
            guests: event.guests
              ?.filter((guest: Guest) => {
                // CRITICAL: Filter out deleted guests - they should not be saved to localStorage
                if (deletedGuestIds.includes(guest.id)) {
                  console.log(`?ƒú½ Filtering out deleted guest from localStorage (fallback): ${guest.firstName} ${guest.lastName} (${guest.id})`);
                  return false;
                }
                return true;
              })
  ...
  crossTabSync.subscribe('rsvp-events-storage', (message) => {
    if (message.type === 'store-update' || message.type === 'force-refresh') {
      ...
      if (message.data?.state) {
        console.log('?ƒôä Cross-tab: Updating events from other tab...');
