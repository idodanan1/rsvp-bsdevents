# 📋 דרישות לשליחת תבנית "aa"

## ✅ מה נדרש לשליחת תבנית "aa" בהצלחה:
...
### 1. **תמונת כותרת (Header Image)** - סטטית! ⚠️
   ...
   - **מה לבדוק:**
     - האם התבנית "aa" ב-Meta Business Manager כוללת תמונת כותרת סטטית?
     - הקוד **לא** ישלח header component עבור תבנית זו

### 2. **8 פרמטרים בגוף ההודעה** - חובה! ⚠️
...
**פתרון:** בדוק שהפרמטרים מועברים בסדר: guest_name, event_type, groom_name, bride_name, event_date, event_time, venue, couple_name

### שגיאה 2: "(#100) Invalid parameter" או "(#132012) Header image mismatch"
**סיבה:** נשלח header component אבל התבנית "aa" כוללת תמונת כותרת סטטית
**פתרון:** הקוד לא אמור לשלוח header component עבור תבנית "aa". בדוק שהלוגים מראים "Skipping header component"

### שגיאה 3: "Template not found"
**סיבה:** התבנית "aa" לא קיימת או לא מאושרת ב-Meta
...
- [ ] כל 8 הפרמטרים קיימים ולא ריקים (guest_name, event_type, groom_name, bride_name, event_date, event_time, venue, couple_name)
- [ ] `templateName` מוגדר ל-'aa'
- [ ] `language` מוגדר ל-'he'
- [ ] Access Token תקין
