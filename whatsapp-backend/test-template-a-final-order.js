...
async function testTemplateAWithCorrectOrder() {
  try {
    const apiUrl = `https://graph.facebook.com/v22.0/${PHONE_NUMBER_ID}/messages`;

    // Template "a" with 9 parameters in CORRECT order (matching Meta template):
    // 1. guest_name
    // 2. event_type
    // 3. bride_name (NOT groom_name!)
    // 4. groom_name
    // 5. event_date
    // 6. event_time
    // 7. venue
        ...
        components: [
          {
            ...
            parameters: [
              ...
              {
                type: 'text',
                text: 'נאל' // Parameter 3: bride_name (BEFORE groom_name!)
              },
              {
                type: 'text',
                text: 'דור' // Parameter 4: groom_name (AFTER bride_name!)
              },
              {
                type: 'text',
    ...
    console.log('📤 בודק שליחת טמפלט "a" עם סדר נכון...');
    console.log('');
    console.log('📋 Payload:');
    ...
    console.log('📋 Order: guest_name, event_type, bride_name, groom_name, event_date, event_time, venue, guest_response_link, couple_name');
    console.log('');
    ...
    if (error.response) {
      ...
      if (errorCode === 132000) {
        ...
        console.error('💡 פרטים:', errorDetails);
        console.error('💡 זה יכול להיות כי:');
        console.error('   1. הטמפלט "a" לא מאושר במטה');
        console.error('   2. יש בעיה עם אחד הפרמטרים');
        console.error('   3. הסדר של הפרמטרים לא תואם למטה');
      }
    } else {
